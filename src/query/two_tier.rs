...
impl TwoTierQueryEngine {
    ...
    /// Helper functions
    async fn find_entity_by_name(&self, _name: &str) -> Result<u32> {
    ...
    async fn get_entity_embedding(&self, _entity_id: u32) -> Result<Vec<f32>> {
        // This would get the actual embedding from the graph
        Ok(vec![0.0; 384]) // Placeholder
    }

    async fn generate_question_embedding(&self, _question: &str) -> Result<Vec<f32>> {
