...
async fn main() {
    ...
    println!("Final activations:");
    println!("A: {}", result.final_activations.get(&key_a).copied().unwrap_or(0.0));
    println!("B: {}", result.final_activations.get(&key_b).copied().unwrap_or(0.0));
    println!("C: {}", result.final_activations.get(&key_c).copied().unwrap_or(0.0));
    println!("D: {}", result.final_activations.get(&key_d).copied().unwrap_or(0.0));
    
    let activation_d = result.final_activations.get(&key_d).copied().unwrap_or(0.0);
    
    println!("\nD activation: {}", activation_d);
