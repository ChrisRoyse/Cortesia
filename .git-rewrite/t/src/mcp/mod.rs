...
impl LLMKGMCPServer {
    ...
    pub async fn handle_request(&self, request: MCPRequest) -> MCPResponse {
        match request.method.as_str() {
            "knowledge_search" => self.handle_knowledge_search(request.params).await,
            "entity_lookup" => self.handle_entity_lookup(request.params).await,
            "find_connections" => self.handle_find_connections(request.params).await,
            "expand_concept" => self.handle_expand_concept(request.params).await,
