...
mod tests {
    ...
    async fn create_hierarchical_storage_system() -> HierarchicalStorageSystem {
        ...
        let memory_tier = Arc::new(RwLock::new(KnowledgeGraph::new()));
        let embedding_tier = EmbeddingStore::new(384).expect("Failed to create embedding store");
        
        let persistent_path = base_path.join("persistent.mmap");
        let persistent_tier = PersistentMMapStorage::new(&persistent_path, 10 * 1024 * 1024) // 10MB
            .expect("Failed to create persistent storage");
        
        let archive_path = base_path.join("archive.mmap");
        let archive_tier = PersistentMMapStorage::new(&archive_path, 50 * 1024 * 1024) // 50MB
