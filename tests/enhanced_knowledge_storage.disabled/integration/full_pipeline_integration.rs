...
mod tests {
    ...
    async fn test_complete_knowledge_lifecycle_integration() {
        ...
        // Step 3: Store in knowledge graph
        {
            ...
            // First add all entities and collect their keys
            for entity in &extraction_result.entities {
                let entity_data = EntityData {
                    type_id: 1,
                    properties: format!(r#"{{"name": "{}"}}"#, entity.name),
                    embedding: vec![0.0; 384],
                };
                let entity_key = graph_lock.add_entity(entity_data)
                    .expect("Failed to add entity");
                entity_keys.insert(entity.id.clone(), entity_key);
            }
            
            // Then add relationships using the entity keys
            for relation in &extraction_result.relations {
                if let (Some(&from_key), Some(&to_key)) = (
                    entity_keys.get(&relation.subject_id),
                    entity_keys.get(&relation.object_id)
                ) {
                    graph_lock.add_relationship(from_key, to_key, 1.0)
        ...
        for (doc_id, doc_text) in documents.iter().enumerate() {
            ...
            // Store in graph with document tracking
            {
                ...
                for entity in &extraction.entities {
                    let entity_data = EntityData {
                        type_id: 1,
                        properties: format!(r#"{{"name": "{}", "source_document": "doc_{}"}}"#, entity.name, doc_id),
                        embedding: vec![0.0; 384],
                    };
    ...
    async fn test_system_wide_performance_integration() {
        ...
        for i in 0..document_count {
            ...
            let graph_start = Instant::now();
            {
                ...
                for entity in &extraction.entities {
                    ...
                    graph_lock.add_entity(entity_data)
                        .expect("Failed to add entity");
                }
            }
        ...
        let entities: Vec<_> = (0..10).map(|i| llmkg::extraction::Entity {
            id: format!("entity_{}", i),
        ...
        let relations: Vec<_> = (0..5).map(|i| llmkg::extraction::Relation {
    ...
    async fn test_cross_component_error_handling_integration() {
        ...
        // Test 2: Graph consistency under errors
        {
            ...
            let result = graph_lock.add_entity(invalid_entity);
            ...
            let valid_entity = EntityData {
                type_id: 1,
                properties: r#"{"name": "TestEntity"}"#.to_string(),
                embedding: vec![0.0; 384],
            };
            let valid_entity_key = graph_lock.add_entity(valid_entity)
    ...
    async fn test_configuration_integration() {
        ...
        let graph = Arc::new(RwLock::new(KnowledgeGraph::new(384).expect("Failed to create graph")));
        let extractor = AdvancedEntityExtractor::new();
        let embedding_store = EmbeddingStore::new(512, 8) // Different dimension
        ...
        let brain_graph_config = Arc::new(BrainEnhancedKnowledgeGraph::new(512).expect("Failed to create brain graph"));
        let orchestrator = CognitiveOrchestrator::new(brain_graph_config, config.clone()).await.expect("Failed to create orchestrator");
        
        // Verify configuration is applied
        let orchestrator_config = orchestrator.get_config();
        assert_eq!(orchestrator_config.max_parallel_patterns, 5);
        assert_eq!(orchestrator_config.default_timeout_ms, 30000);
        assert_eq!(orchestrator_config.enable_adaptive_selection, true);
        assert_eq!(orchestrator_config.performance_tracking, true);
                ...
                // Add to graph in batches
                {
                    ...
                    for (doc_idx, extraction) in batch.iter().enumerate() {
                        for entity in &extraction.entities {
                            let entity_data = EntityData {
                                type_id: 1,
                                properties: format!(r#"{{"name": "{}", "batch": "{}"}}"#, entity.name, load_level),
                                embedding: vec![0.0; 384],
                            };
